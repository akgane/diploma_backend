...
async def add_inventory_item(data: AddInventoryItemRequest, db: AsyncIOMotorDatabase = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def get_inventory(status_filter: str | None = Query(None, alias="status"), db: AsyncIOMotorDatabase = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def get_expiring(days: int = Query(3, ge=1, le=30), db: AsyncIOMotorDatabase = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def get_inventory_item(item_id: str, db: AsyncIOMotorDatabase = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def update_inventory_item(item_id: str, data: UpdateInventoryItemRequest, db: AsyncIOMotorDatabase = Depends(get_db), user: dict = Depends(get_current_user)):
